...
export function PosCajaHistorial() {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-inner ${isAbierta ? 'bg-brand-blue/10 dark:bg-blue-950/60 text-brand-blue dark:text-blue-400' : 'bg-slate-100 dark:bg-neutral-800 text-slate-500 dark:text-neutral-400'}`}>
            ...
            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-2">
                ...
                {isAbierta ? (
                  <Badge variant="default" className="text-[9px] uppercase tracking-wider py-0 px-1.5 bg-brand-blue text-white h-4">Actual</Badge>
                ) : (
                  <Badge variant="secondary" className="text-[9px] uppercase tracking-wider py-0 px-1.5 dark:bg-neutral-800 dark:text-neutral-300 h-4">Cerrada</Badge>
                )}
              </div>
        ...
        return (
          <div className="flex items-center gap-4">
            <div className="flex flex-col">
              ...
              <span className="font-semibold text-slate-700 dark:text-neutral-300 text-sm tabular-nums">{formatCurrency(Number(caja.montoInicial))}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] text-emerald-500/80 dark:text-emerald-400/90 font-bold uppercase flex items-center gap-1">
                <TrendingUp className="w-3 h-3" /> In
              </span>
              <span className="font-semibold text-slate-700 dark:text-emerald-400 text-sm tabular-nums">{formatCurrency(ingresos)}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] text-rose-500/80 dark:text-rose-400/90 font-bold uppercase flex items-center gap-1">
                <TrendingDown className="w-3 h-3" /> Out
              </span>
              <span className="font-semibold text-slate-700 dark:text-rose-400 text-sm tabular-nums">{formatCurrency(egresos)}</span>
            </div>
          </div>
        )
      }
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-col items-start justify-center min-w-[100px]">
            <span className="text-[10px] text-slate-400 dark:text-neutral-500 font-bold uppercase tracking-wider mb-0.5">
        ...
        return (
          <div className="flex justify-end pr-2">
            <Button 
              variant="ghost" 
              size="icon"
              className="w-8 h-8 rounded-full hover:bg-slate-100 dark:hover:bg-neutral-800 text-slate-400 dark:text-neutral-400 hover:text-brand-blue dark:hover:text-blue-400"
              onClick={(e) => {
                e.stopPropagation();
                router.push(`/pos/caja/${row.original.id}`);
              }}
            >
