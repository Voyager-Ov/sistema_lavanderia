...
export interface CobrarPedidosSheetProps {
  open: boolean
...
export function CobrarPedidosSheet({
  open,
  onOpenChange,
  ...
  const effectiveClienteId = useMemo(() => {
    if (propClienteId) return propClienteId
    if (pedidos && pedidos.length > 0) {
      return pedidos[0].clienteId || pedidos[0].cliente?.id || 0
    }
    return 0
  }, [propClienteId, pedidos])

  const effectiveClienteNombre = useMemo(() => {
    if (propClienteNombre) return propClienteNombre
    if (pedidos && pedidos.length > 0) {
      const c = pedidos[0].cliente
      if (c) {
  ...
  const totalPedidos = useMemo(() => {
    ...
    const validos = (pedidos || []).filter(p => !isPedidoCancelado(p))
  ...
  const vuelto = remanenteAPagar > 0 && montoNum > remanenteAPagar ? montoNum - remanenteAPagar : 0
  ...
  const handleCobrar = async () => {
    ...
    const tieneCancelados = pedidos.some(p => {
      const est = (typeof p.estado === "object" ? p.estado?.nombre : p.estado) || ""
    ...
    setLoading(true)
    try {
      const pedidosIds = pedidos.map((p) => p.id || p.numeroPedido)

      if (effectiveClienteId) {
        await cobrarPedidosCliente(effectiveClienteId, {
          pedidosIds,
          ...
          observaciones: `Cobro en mostrador de ${pedidos.length} pedido(s)`
        })
      } else {
        // Fallback para pedido sin cliente directo registrado
        for (const p of pedidos) {
          const pId = p.id || p.numeroPedido
          await registrarPago({
            pedidoId: pId,
            metodoPagoId: remanenteAPagar > 0 ? parseInt(selectedMetodo) : undefined,
            monto: parseFloat(p.total?.toString() || "0"),
      ...
      toast.success(`Pago registrado exitosamente para ${pedidos.length} pedido(s).`)
      onSuccess()
      onOpenChange(false)
    } catch (error: any) {
      toast.error(
  ...
  const esUnicoPedido = pedidos.length === 1
  const pedidoUnico = esUnicoPedido ? pedidos[0] : null
  const ticketLabel = pedidoUnico ? `#${pedidoUnico.codigoSeguimiento || pedidoUnico.numeroPedido || pedidoUnico.id}` : `${pedidos.length} pedido(s)`
      ...
      <ResponsiveSheetContent>
        <ResponsiveSheetHeader className="mb-6 text-left">
          <ResponsiveSheetTitle className="text-xl font-black text-gray-900 dark:text-neutral-100">
            {esUnicoPedido ? `Cobrar Pedido ${ticketLabel}` : "Cobrar Pedidos"}
          </ResponsiveSheetTitle>
          <ResponsiveSheetDescription className="text-xs text-gray-500 dark:text-neutral-400">
            Registra el pago para {ticketLabel} de {effectiveClienteNombre}.
          </ResponsiveSheetDescription>
        </ResponsiveSheetHeader>
        ...
        <div className="flex-1 space-y-5">
          ...
          <div className="bg-slate-50 dark:bg-neutral-800/60 rounded-2xl p-4 border border-gray-100 dark:border-neutral-700/60 flex flex-col gap-2">
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-bold text-gray-900 dark:text-neutral-100">{effectiveClienteNombre}</span>
            </div>
            <div className="flex justify-between items-center text-xs text-gray-500 dark:text-neutral-400">
              <span>Selección</span>
              <span className="font-bold text-gray-900 dark:text-neutral-100">{ticketLabel}</span>
            </div>
            <div className="flex justify-between items-center text-lg mt-2 pt-2 border-t border-gray-200 dark:border-neutral-700">
              <span className="text-gray-600 dark:text-neutral-300 font-medium">Total Pedido(s)</span>
