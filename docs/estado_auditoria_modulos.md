...
| 📊 **`reportes`** | ✅ **Auditado & Validado** | Validadores Fail-Fast (`reportes.validator.js`), eliminación de multiplicadores ficticios (`* 12`), KPIs hardcodeados (`85%`, `12h`, `24h`), promedios planos por empleado y donas falsas. | Cliente `reportes.api.ts` 1:1, migración de `usePedidosReport`, `useServiciosReport` y `useEmpleadosReport` eliminando `any`. | `back2/src/tests/reportes/audit_reportes_module.js` |
