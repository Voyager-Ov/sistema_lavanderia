...
import { useClienteDetail } from "@/app/admin/clientes/hooks/useClienteDetail"
...
export default function PosClienteDetailPage() {
  ...
  if (!cliente) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center h-full p-8 text-center">
        ...
        <Button onClick={() => router.push("/pos/clientes")} className="rounded-full px-8">
          Volver al listado
        </Button>
      </div>
    )
      ...
      <div className="stagger-in flex items-center justify-between px-4 sm:px-8 pt-6 pb-4 border-b border-gray-100 bg-white/80 backdrop-blur sticky top-0 z-20">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={() => router.push(`/pos/clientes/${cliente.id}/editar`)}
          >
      ...
      <div className="flex-1 p-4 sm:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 items-start">
        ...
        <div className="stagger-in lg:col-span-1 space-y-4">
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-7 flex flex-col items-center text-center overflow-hidden relative group">
            <div className="absolute -top-16 -right-16 w-40 h-40 bg-brand-blue/10 rounded-full group-hover:scale-125 transition-transform duration-700 z-0" />
            <div className="relative z-10 w-24 h-24 bg-gradient-to-br from-brand-blue to-blue-500 text-white rounded-[1.75rem] flex items-center justify-center text-4xl font-black shadow-lg mb-5 rotate-3 group-hover:rotate-0 transition-transform duration-300">
            ...
            <div className="mt-5 w-full space-y-2">
              {cliente.telefono && (
                ...
                >
                  <div className="p-2 bg-white rounded-xl shadow-sm shrink-0"><Phone className="w-4 h-4 text-brand-blue" /></div>
              ...
              {cliente.email && (
                <div className="flex items-center gap-3 text-gray-600 font-medium bg-gray-50 p-3 rounded-2xl">
                  <div className="p-2 bg-white rounded-xl shadow-sm shrink-0"><Mail className="w-4 h-4 text-brand-blue" /></div>
              ...
              {chartData.length > 1 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                    ...
                    />
                    <Area type="monotone" dataKey="total" stroke="#2563eb" strokeWidth={2.5} fillOpacity={1} fill="url(#gradTotal)" dot={false} activeDot={{ r: 5, strokeWidth: 0 }} />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-gray-100">
              ...
              <Button 
                variant="ghost" 
                size="sm" 
                className="text-xs font-bold text-brand-blue hover:text-blue-700 hover:bg-blue-50 rounded-full px-4"
                onClick={() => {
