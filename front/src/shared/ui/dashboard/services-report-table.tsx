...
export function ServicesReportTable({ data, className }: ServicesReportTableProps) {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <span className="font-bold text-gray-900 w-8 text-right">{row.original.porcentajeVentas}%</span>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center">
          {row.original.tendencia === "up" && <ArrowUp className="w-5 h-5 text-green-500" strokeWidth={3} />}
          {row.original.tendencia === "down" && <ArrowDown className="w-5 h-5 text-red-500" strokeWidth={3} />}
          {row.original.tendencia === "flat" && <Minus className="w-5 h-5 text-gray-300" strokeWidth={3} />}
        </div>
      ),
    }
  ], []);
  ...
  return (
    <div className={`bg-white rounded-[2.5rem] p-4 sm:p-6 border border-gray-100 shadow-sm flex flex-col ${className || ''}`}>
